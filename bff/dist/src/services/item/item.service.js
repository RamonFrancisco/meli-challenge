...
class ItemService {
    ...
    async findById(id) {
        ...
        if (resultItem) {
            let categories = [];
            const item = this.itemAdapter.adaptItem({ ...resultItem.data, description });
    ...
    async search(query, offset, limit) {
        ...
        if (result) {
            ...
            meliCategories = meliCategories && meliCategories.values.length && meliCategories.values[0].path_from_root.length ? meliCategories.values[0].path_from_root : [];
            categories = meliCategories.map((category) => category.name);
            return {
                items,
                categories,
